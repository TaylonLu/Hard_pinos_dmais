...
module elm_fsm (
    ...
    // --- Disparo e status ---
    input  wire start,         // pulso (1 ciclo) para iniciar inferência
    output reg  done,          // 1 quando predição está pronta
    output reg  [3:0] pred,    // classe predita

    // --- Interface com mem_image (leitura) ---
    output reg  [9:0]  img_addr,   // índice 0..783
    input  wire [7:0]  img_data,   // pixel Q8.0

    // --- Interface com mem_win (leitura W_in) ---
    output reg  [17:0] win_addr,   // índice = n*784 + p  (máx 256*784-1=200703)
    input  wire signed [15:0] win_data,

    // --- Interface com mem_bias (leitura) ---
    output reg  [7:0]  bias_addr,  // índice 0..N_HIDDEN-1
    input  wire signed [15:0] bias_data,

    // --- Interface com mem_beta (leitura beta) ---
    output reg  [17:0] beta_addr,  // índice = c*N_HIDDEN + k
    ...
    input  wire [9:0]  mac_addr,   // endereço interno do mac (monitoramento)
    input  wire        mac_done,
	...
	// Constantes
	 parameter N_HIDDEN  = 4;    // número de neurônios ocultos
    parameter N_CLASSES = 10;   // número de classes de saída
    parameter IMG_SIZE  = 784;   // número de pixels (sempre 28×28=784)
    ...
    // Estados da FSM
    localparam [3:0]
        IDLE          = 4'd0,   // aguardando start
        H_RESET_MAC   = 4'd1,   // reseta mac antes de cada neurônio oculto
        H_RUN         = 4'd2,   // acumula 784 produtos para neurônio n
        H_WAIT        = 4'd3,   // aguarda mac_done (1 ciclo extra de latência)
        H_WRITE       = 4'd4,   // grava h[n] = sigmoid(mac_saida)
        O_RESET_MAC   = 4'd5,   // reseta mac antes de cada classe
        O_RUN         = 4'd6,   // acumula N_HIDDEN produtos para classe c
        O_WAIT        = 4'd7,   // aguarda mac_done
        O_WRITE       = 4'd8,   // grava y[c] e alimenta argmax
        ARG_FEED      = 4'd9,   // envia todos os y[c] ao argmax (pós-cálculo)
        ARG_WAIT      = 4'd10,  // aguarda argmax.done
        DONE_ST       = 4'd11,  // predição pronta
        ERROR_ST      = 4'd12;  // estado de erro (sem saída válida)

    reg [3:0] state;

    // Contadores
    reg [7:0]  n_cnt;      // neurônio oculto atual  (0..N_HIDDEN-1)
    reg [3:0]  c_cnt;      // classe de saída atual  (0..N_CLASSES-1)
    reg [9:0]  p_cnt;      // pixel/hidden atual     (0..783 ou 0..N_HIDDEN-1)

    // Registrador de resultados de saída para re-alimentar o argmax
    // (necessário porque o mac só produz um resultado por vez)
    reg signed [15:0] y_scores [0:N_CLASSES-1];

    // Pixel convertido para Q4.12: pixel << 4
